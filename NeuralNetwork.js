...
var ValX = ValData.slice([0, 0], [-1, 12])
var ValY = ValData.slice([0, 12], [-1, 1])
...
function KNN(ValX, x,y){
  var new_x = tf.tensor([0])
  var tt = tf.tensor([0])
  for (var i = 0; i <= (ValX.shape[0] - 1); i++) {
    var new_x = tf.tensor([0])
    var new_ValX = ValX.slice(i, 1, -1)
    var DistanceKindof = tf.sub(x, new_ValX)
    for (var iterator = 0; iterator < DistanceKindof.shape[0]; iterator++) {
      var CosineDistanceSingle = tf.norm(DistanceKindof.slice(iterator, 1, -1))
      CosineDistanceSingle = CosineDistanceSingle.dataSync()
      CosineDistanceSingle = Array.from(CosineDistanceSingle)
      CosineDistanceSingle = tf.tensor(CosineDistanceSingle)
      new_x = tf.concat([new_x, CosineDistanceSingle])
    }
    new_x = new_x.dataSync()
    new_x = Array.from(new_x)
    //var newer_x = new_x.splice(0, 1)
    newer_x = tf.tensor(new_x)
    newer_x = newer_x.slice(1, (newer_x.shape[1]), 1)
    newer_x = newer_x.reshape([newer_x.shape[0], 1])
    newer_x = tf.concat([newer_x, y], 1)
    var fa = newer_x.gather([0], 1).reshape([-1]);
    var ifnd = tf.topk(fa, newer_x.shape[0]).indices
    newer_x = newer_x.gather(ifnd.reverse(), 0)
    //newer_x = tf.tensor(newer_x)
    newer_x = newer_x.slice(0, 1, 1)
    newer_x = newer_x.slice([0, 1]).reshape([1])
    tt = tf.concat([tt, newer_x])
  }
  var knn_predictions_for_residual_boosting = tt.dataSync()
  knn_predictions_for_residual_boosting = Array.from(knn_predictions_for_residual_boosting)
  knn_predictions_for_residual_boosting = knn_predictions_for_residual_boosting.splice(1, knn_predictions_for_residual_boosting.length)
  knn_predictions_for_residual_boosting = tf.tensor([knn_predictions_for_residual_boosting])
  knn_predictions_for_residual_boosting = knn_predictions_for_residual_boosting.reshape([knn_predictions_for_residual_boosting.shape[1], 1])
  return knn_predictions_for_residual_boosting
}
...
function PrepareDataset(x) {
  ...
  var smoking = Dataset.slice([0, 10], [-1, 1])
  ...
  for (i = 0; i < x.shape[0]; i++) {
    var x_i = tf.concat([age.slice([i, 0], [1, 1]), anaemia.slice([i, 0], [1, 1]), creatinine_phosphokinase.slice([i, 0], [1, 1]), diabetes.slice([i, 0], [1, 1]), ejection_fraction.slice([i, 0], [1, 1]), high_blood_pressure.slice([i, 0], [1, 1]), platelets.slice([i, 0], [1, 1]), serum_creatinine.slice([i, 0], [1, 1]), serum_sodium.slice([i, 0], [1, 1]), sex.slice([i, 0], [1, 1]), smoking.slice([i, 0], [1, 1]), time.slice([i, 0], [1, 1])]).squeeze()
    var values = x_i.dataSync()
    var tensor = Array.from(values)
    var x_u = tf.tensor([tensor])
...
model.fit(x, y, {
  ...
  callbacks: {
    onEpochEnd: async (epoch, logs) => {
      ...
      console.log("Loss: " + logs.loss + " accuracy: " + logs.acc + " validation accuracy: " + logs.val_acc + " validation loss: " + logs.val_loss)
    },
  }
}).then(h => {
  console.log("Training Complete")
  y = tf.sub(y, model.predict(x))
  var knn_predictions_for_residual_boosting = KNN(ValX, x,y)
  /*
  new_x = new_x.dataSync()
  new_x = Array.from(new_x)
  new_x.splice(0, 1)
  new_x = tf.tensor(new_x)
  new_x = new_x.reshape([new_x.shape[0], 1])
  new_x = tf.concat([new_x, y])
  new_x.print()
  */
  console.log("Predictions")
  console.log("ŷ")
  var final_predictions = knn_predictions_for_residual_boosting
  var final_predictions = tf.round(tf.add(model.predict(ValX), knn_predictions_for_residual_boosting))
  console.log("ŷ for nn")
  model.predict(ValX).print()
  tf.sub(ValY, model.predict(ValX)).print()
  console.log("ŷ for KNN")
  knn_predictions_for_residual_boosting.print()

  final_predictions = final_predictions.reshape([final_predictions.shape[0]])
  console.log("Accuracy")
  tf.metrics.binaryAccuracy(final_predictions, ValY.reshape([ValY.shape[0]])).print()
